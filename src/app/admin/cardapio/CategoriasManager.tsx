...
export default function CategoriasManager({ tenantId }: { tenantId: string }) {
  ...
  const handleOpenEdit = (category: Category) => {
    ...
    setFormSortOrder(category.sortOrder.toString());
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setError(errData.error || "Ocorreu um erro ao salvar a categoria.");
      }
    } catch {
    ...
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <p className="text-xs text-[#6B5A4B] font-light mt-1">
          Organize o cardápio por categorias. Use a ordem para controlar a exibição.
        </p>
        <button
                ...
                <tr key={category.id} className="hover:bg-[#FAF7F2]/40 transition-colors">
                  <td className="p-4 pl-6">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="font-semibold text-[#2E251B]">{category.name}</p>
                  ...
                  <td className="p-4 font-mono text-[#6B5A4B]">{category.sortOrder}</td>
          ...
          <div className="bg-white border border-[#EBE2D5] w-full max-w-lg rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              ...
              <div className="flex items-center justify-between bg-[#FAF7F2] border border-[#EBE2D5] p-4 rounded-xl shadow-inner mt-2">
                <div>
                  <h4 className="text-xs font-bold text-amber-950">Categoria Ativa</h4>
                  <p className="text-[10px] text-[#6B5A4B] mt-0.5 font-light">
                    Se desativada, a categoria some do cardápio digital do cliente.
                  </p>
                </div>
                <input
                  type="checkbox"
