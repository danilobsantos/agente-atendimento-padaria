...
6. NEVER set intent to 'confirmar_pedido' if either Address or Payment is 'Not provided'. Instead, set the intent to 'generico' and politely ask the customer for the missing information (address or payment method).
${session.activeOrderId ? `7. CONTEXTO: O cliente já tem um pedido ativo sendo preparado. Se ele pedir novos itens, adicione usando a intent 'adicionar_itens' e avise que foram adicionados ao pedido atual.` : ""}
